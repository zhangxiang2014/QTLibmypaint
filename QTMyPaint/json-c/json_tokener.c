					  ...
					  const char *str, int len)
{
  ...
  redo_char:
	...
	if ((!(tok->flags & JSON_TOKENER_STRICT) &&
               _strnicmp(json_inf_str, infbuf, size_inf) == 0) ||
	...
	if((!(tok->flags & JSON_TOKENER_STRICT) &&
      _strnicmp(json_null_str, tok->pb->buf, size) == 0)
	  ...
	  if (tok->st_pos == json_null_str_len) {
	    ...
	    goto redo_char;
               ...
               _strnicmp(json_nan_str, tok->pb->buf, size_nan) == 0) ||
	...
	if((!(tok->flags & JSON_TOKENER_STRICT) &&
       _strnicmp(json_true_str, tok->pb->buf, size1) == 0)
	  ...
	  if(tok->st_pos == json_true_str_len) {
       ...
       _strnicmp(json_false_str, tok->pb->buf, size2) == 0)
